...
function genMeetingLink(datetimeISO: string) {
  ...
  return `https://meet.example.com/${id}?t=${encodeURIComponent(
    datetimeISO
  )}`;
}

const EditAppointmentForm = forwardRef<EditAppointmentFormHandle>(
  (_props, ref) => {
    ...
    const [modality, setModality] = useState<"virtual" | "presencial">(
      "virtual"
    );
    ...
    const [originalAppointment, setOriginalAppointment] =
    ...
    useEffect(() => {
      ...
      const originalDay = originalDate
        .getDate()
        .toString()
        .padStart(2, "0");
      ...
      const hasClientChanges =
        client.trim() !== originalAppointment.client;
      const hasContactChanges =
        contact.trim() !== originalAppointment.contact;
      const hasDescriptionChanges =
        (description || "").trim() !==
        (originalAppointment.description || "").trim();
      const hasModalityChanges =
      ...
      if (modality === "presencial") {
        ...
        hasLocationOrLinkChanges =
          (meetingLink || "").trim() !==
          (originalAppointment.meetingLink || "").trim();
      }
      ...
      () => ({
        open: (appointmentData: ExistingAppointment) => {
          const normalized: "virtual" | "presencial" =
            appointmentData.modality === "virtual"
              ? "virtual"
              : "presencial";
          ...
          setLat(
            Number.isFinite(latNum as number)
              ? (latNum as number)
              : undefined
          );
          const lonNum = appointmentData.lon ?? undefined;
          setLon(
            Number.isFinite(lonNum as number)
              ? (lonNum as number)
              : undefined
          );
    ...
    async function handleSubmit(e: React.FormEvent) {
      ...
      const originalDate = new Date(
        originalAppointment?.datetime || datetime
      );
      ...
      const payload: any = {};
      if (!originalAppointment)
        return setMsg("Error: datos originales no disponibles");
      ...
      if (modality === "presencial") {
        ...
        if (
          lat !== undefined &&
          lat !== (originalAppointment.lat ?? undefined)
        )
